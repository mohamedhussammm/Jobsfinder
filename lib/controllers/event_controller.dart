...
final publishedEventsProvider = FutureProvider.autoDispose.family<List<EventModel>, int>(
  (ref, page) async {
...
class EventController {
  ...
  Future<Result<EventModel>> createEventRequest({
    ...
    try {
      ...
      if (endTime.isBefore(startTime)) {
        return Error(ValidationException(message: 'End time must be after start time'));
      }
  ...
  /// Admin rejects event
  Future<Result<EventModel>> rejectEvent(String eventId, {String? reason}) async {
    try {
      final response = await _supabase
          .from(SupabaseTables.events)
          .update({
